...
try {
  ...
  //remove some headers
  $requestHeaders = array_diff_key(
    $requestHeaders, ['Host' => 1, 'Origin' => 1, 'Content-Length' => 1]
  );
  ...
  if ($curlInfo['http_code'] == 200) {
    ...
    try {
      ...
      $saveFiles = isset($serverConfig['saveFiles']) ? $serverConfig['saveFiles'] : true;
      ...
      if ($saveFiles) {
        ...
        if ($_FILES) {
          ...
          foreach ($_FILES as $fileInfo) {
            ...
            $thumbName = preg_replace(
              '/\.(.+)$/', '.thumb.$1', $cleanName
            );
            $thumbFile = $dir . '/' . $counter . '-' . $thumbName;
            try {
              if (Volcano_Tools_Image::resizeImage(
                $dstFile, $thumbFile, 200, 200
              )
              ) {
                list($thumbWidth, $thumbHeight) = getimagesize(
                  $thumbFile
                );
                $file['thumbUri'] = $url . '/' . $counter . '-'
                  . $thumbName;
        ...
        $update = $pdo->prepare(
          "UPDATE items SET files = ? WHERE id = ?"
        );
        $update->execute([json_encode($files), $id]);
      }
      file_put_contents(
        $dir . '/request-headers.json', json_encode($requestHeaders)
      );
      if (is_array($post)) {
      ...
      elseif (strlen($post)) {
        file_put_contents(
          $dir . '/post.json', json_encode(['_allpost' => $post])
        );
      }
      file_put_contents(
        $dir . '/response-headers.json', json_encode($responseHeaders)
      );
